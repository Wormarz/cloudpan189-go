...
部分账号开启了扫码安全验证, 使用 APP 密码登录会失败(登录页请求超时)。
此时在登录流程中确认改用扫码登录即可, 程序会把二维码**直接显示在终端**并轮询扫码结果,
使用手机天翼云盘 App 的"扫码登录"功能扫码并确认后即可完成登录:

```
cloudpan189-go login
请输入用户名(手机号/邮箱/别名), 回车键提交 > 1234567
...
账号可能需要扫码安全验证, 是否改用扫码登录? (y/n)
> y
请在手机上打开天翼云盘App, 使用"扫码登录"扫描以下二维码完成登录:
████ ▄▄▄▄▄ █▄█ ▀█▀▀▀▀█▀ █ ▀█▀ ▀▄█ █ ██ ▄▄▄▄▄ ████
████ █   █ █▀▀█▄ ▀ █  ▄▀▄▀▄▀██▄ ▄▀▀▄ █ █   █ ████
...  (终端直接显示二维码, 手机对着屏幕扫码即可)
```

若终端渲染异常(或 Windows 的 cmd 控制台), 二维码图片会保存在临时目录供打开查看。

等待扫码过程中终端会显示剩余时间倒计时; 按 `Ctrl+C` 可**取消本次扫码**(不会退出程序)。
二维码过期或等待超时(最长 3 分钟)后会自动生成新的二维码再扫一次。

> 会话恢复: 重新进入程序时会用已保存的会话直接校验并恢复登录状态,
> 会话仍有效则无需重新输入密码; 会话过期后自动登录, 密码登录失败时同样会引导扫码登录。
> 扫码登录成功后保存的是会话凭证(没有密码), 之后进入程序会自动恢复, 无需再登录。
...
大于 200MiB 的文件自动使用网页分片上传。初始化返回 HTTP 500 时，会使用现有 accessToken 换取新的上传会话并重试一次；恢复失败时请重新 login。上传专用会话不会覆盖已保存的认证配置。大文件覆盖通过上传完成后的提交操作执行。
