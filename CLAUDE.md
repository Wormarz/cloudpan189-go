...
2. **`cmder/`** — CLI 胶水。`cmder_helper.go` 全局函数 `ReloadConfigFunc`/`SaveConfigFunc`（多数命令的 `Before`/`After` 钩子）、`DoLoginHelper`（含验证码流程）、`TryLogin`。`cmdliner/`（REPL 行编辑与 shell 风格参数解析）、`cmdtable/`（表格渲染，包一层 tablewriter）、`cmdutil/`（工作目录、地址、转义 escaper、jsonhelper）。
...
7. **`internal/file/downloader/` / `uploader/`** — 底层 HTTP 分片传输引擎。
   ...
   - uploader：`MultiUpload` 接口 + 分块上传。**天翼云盘单文件只支持单线程上传、支持断点续传**（`CmdUpload` 里 `Parallel=1, NoSplitFile=true`），上传中断现场存 `cloud189_uploading.json`。
